"use client";

import Link from "next/link";
import { AlertTriangleIcon, BellIcon, CalendarClockIcon, UsersIcon } from "lucide-react";
import { api } from "@/lib/api";
import { useAuth } from "@/lib/auth-context";
import { formatRupees } from "@/lib/format";
import { useResource } from "@/lib/use-resource";
import type { DashboardSummary } from "@/lib/types";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";

export default function DashboardPage() {
  const { activeGym, user } = useAuth();
  const gymId = activeGym!.id;

  const { data, error, loading } = useResource<DashboardSummary>(
    () => api.get<DashboardSummary>(`/gyms/${gymId}/dashboard/summary`),
    `dashboard:${gymId}`,
  );

  const needsAttention = (data?.overdue ?? 0) + (data?.dueSoon ?? 0);

  return (
    <div className="space-y-5 px-4 pt-6">
      <header>
        <p className="text-sm text-muted-foreground">
          Namaste{user?.name ? `, ${user.name.split(" ")[0]}` : ""}
        </p>
        <h1 className="text-xl font-semibold tracking-tight">{activeGym!.name}</h1>
      </header>

      {error && (
        <Alert variant="destructive">
          <AlertTriangleIcon />
          <AlertTitle>Could not load the dashboard</AlertTitle>
          <AlertDescription>{error.message}</AlertDescription>
        </Alert>
      )}

      {/* The primary action, placed above the numbers: the point of the app is
          sending reminders, not reading statistics. */}
      {needsAttention > 0 && !loading && (
        <Card className="border-primary/30 bg-primary/5">
          <CardContent className="flex items-center justify-between gap-3 py-4">
            <div>
              <p className="font-medium">
                {needsAttention} {needsAttention === 1 ? "member" : "members"} ko reminder bhejna hai
              </p>
              <p className="text-sm text-muted-foreground">
                {data!.overdue} overdue · {data!.dueSoon} due soon
              </p>
            </div>
            <Button render={<Link href="/reminders" />} size="sm" className="h-11 shrink-0">
                <BellIcon className="size-4" />
                Bhejo
              </Button>
          </CardContent>
        </Card>
      )}

      <div className="grid grid-cols-2 gap-3">
        <Stat
          label="Overdue"
          value={data?.overdue}
          loading={loading}
          icon={<AlertTriangleIcon className="size-4" />}
          tone="danger"
        />
        <Stat
          label="Due soon"
          value={data?.dueSoon}
          loading={loading}
          icon={<CalendarClockIcon className="size-4" />}
        />
        <Stat
          label="Active members"
          value={data?.activeMembers}
          loading={loading}
          icon={<UsersIcon className="size-4" />}
        />
        <Stat
          label="Pending amount"
          value={data ? formatRupees(data.pendingAmount) : undefined}
          loading={loading}
        />
      </div>

      <Card>
        <CardContent className="py-4">
          <p className="text-sm text-muted-foreground">Is mahine collect hua</p>
          {loading ? (
            <Skeleton className="mt-1 h-8 w-32" />
          ) : (
            <p className="text-2xl font-semibold">
              {formatRupees(data?.collectedThisMonth ?? 0)}
            </p>
          )}
        </CardContent>
      </Card>

      <Button render={<Link href="/members" />} variant="outline" size="lg" className="h-12 w-full">Saare members dekho</Button>
    </div>
  );
}

function Stat({
  label,
  value,
  loading,
  icon,
  tone,
}: {
  label: string;
  value: number | string | undefined;
  loading: boolean;
  icon?: React.ReactNode;
  tone?: "danger";
}) {
  return (
    <Card>
      <CardContent className="py-4">
        <div className="flex items-center gap-1.5 text-muted-foreground">
          {icon}
          <span className="text-xs">{label}</span>
        </div>
        {loading ? (
          <Skeleton className="mt-1.5 h-7 w-14" />
        ) : (
          <p
            className={
              tone === "danger" && Number(value) > 0
                ? "text-2xl font-semibold text-destructive"
                : "text-2xl font-semibold"
            }
          >
            {value ?? "—"}
          </p>
        )}
      </CardContent>
    </Card>
  );
}
